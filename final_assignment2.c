...
void move(struct Players *Player, int x, int playernumber, int slotnum, int *choi);
...
int main(void)
{
	...
	int PlayerNum, j, slot_no, i, random, temp, counter, choice;
	int max=100, v=0, second=99, r=0, k=0, l;
	int dif;
	...
	for(i=0; i<PlayerNum; i++)	//assign players to slots
	{
		...
		boost(&Player[i], &slot[Player[i].Place]);	//don't need slot[i] here it's not changeing
	}
...
*/		//if move -> move function
		if(choice==1)
		{
			printf("\n%s Place is %d", Player[i].Name, Player[i].Place);
			deboost(&Player[i], &slot[Player[i].Place-1]);
			printf( "\nPlayer [%d]: %s\nPlayer Type: %s\nStrength: %d\nMagic: %d\nDexterity: %d\nLuck: %d\nSmartness: %d\n", i+1, Player[i].Name, Player[i].Race, Player[i].Strength, Player[i].MagicSkills, Player[i].Dexterity, Player[i].Luck, Player[i].Smartness);
			move(&Player[0], i, PlayerNum, slot_no, &choice);
			boost(&Player[i], &slot[Player[i].Place-1]);
			printf( "\nPlayer [%d]: %s\nPlayer Type: %s\nStrength: %d\nMagic: %d\nDexterity: %d\nLuck: %d\nSmartness: %d\n", i+1, Player[i].Name, Player[i].Race, Player[i].Strength, Player[i].MagicSkills, Player[i].Dexterity, Player[i].Luck, Player[i].Smartness);
			printf("\n%s Place is %d", Player[i].Name, Player[i].Place);
		}
...
void move(struct Players *Player, int x, int playernumber, int slotnum, int *choi)		//call this	
{
	int l=0, right=0, left=0, c;
	while(l<playernumber)
	{
		
		if(Player[x].Place+1==Player[l].Place || (Player[x].Place==slotnum) && right==0)
		{
			printf("You can't move forward\n");
			right++;
		}
		if(Player[x].Place-1==Player[l].Place || Player[x].Place==1 && left==0)
		{
